...
def _replace_to_test_root(test_added_path: Path) -> Path:
    ROOT_PATH: Path = Path('project', 'sparta')
    root_path: Path = path_absolute(ROOT_PATH)
    return Path(
        root_path,
        'tests',
        test_added_path.relative_to(root_path)
    )
...
def _check_test_path(call_context: PathPair) -> bool:
    ...
    if not call_context['src'].name.startswith(TEST_HEAD):
        test_module_path: Path = _replace_to_test_root(
            _replace_file_name(TEST_HEAD, call_context['module']))

        if test_module_path.exists():
            call_context.update({'module': test_module_path})
            return True
